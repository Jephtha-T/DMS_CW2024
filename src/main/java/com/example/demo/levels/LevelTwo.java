...
public class LevelTwo extends LevelParent {
	...
	protected void spawnEnemyUnits() {
		...
		for (int i = 0; i < TOTAL_ENEMIES - currentNumberOfEnemies; i++) {
			if (new java.security.SecureRandom().nextDouble() < ENEMY_SPAWN_PROBABILITY) {
				...
				if (randomValue < 0.6) { // 60% chance for normal EnemyPlane
					...
					newEnemy = BomberPlane.createInRightHalf(getScreenWidth());
	...
	protected void spawnItems() {
		...
		for (int i = 0; i < TOTAL_ITEMS - currentNumberOfItems; i++) {
			if (new java.security.SecureRandom().nextDouble() < ITEM_SPAWN_PROBABILITY) {
				...
				if (itemTypeProbability < 1.0 / 2.0) {
					// Spawn MultiShotItem
